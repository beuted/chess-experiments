...
type PreparationProps = {
  hydratedArchives: HydratedChessComArchive[] | undefined;
};

export function Preparation(props: PreparationProps) {
  const [chapterLines, setChapterLines] = useState<
    { title: string | undefined; fen: string | undefined; lines: string[] }[]
  >([]);
  const chapterOrientationStored = JSON.parse(
    localStorage.getItem("chapterOrientation") || "[]"
  ); // Stored in localstorage
  const [chapterOrientation, setChapterOrientation] = useState<
    { orientation: "white" | "black" | null }[]
  >(chapterOrientationStored);
  ...
  const [availableBlackStudy, setAvailableBlackStudy] =
    useState<boolean>(false);
  const [availableWhiteStudy, setAvailableWhiteStudy] =
    useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (studyUrl) fetchStudy(studyUrl);
  ...
  useEffect(() => {
    ...
    if (chapterOrientation && chapterOrientation.length > 0) {
      ...
      const newChapterOrientation: { orientation: "white" | "black" | null }[] =
        chapterMoves.map((chapter) => {
          let moveIndex = 0;
          while (true) {
      ...
      localStorage.setItem(
        "chapterOrientation",
        JSON.stringify(newChapterOrientation)
      );
    }
  }, [chapterLines]);

  useEffect(() => {
    if (!nextPlayerMove || !showTips) return;

    setCustomSquareStyles({
      [nextPlayerMove.from]: { backgroundColor: "green" },
    });
  }, [showTips]);

  useEffect(() => {
    if (!nextPlayerMove || !showSolution) return;

    setCustomArrows([[nextPlayerMove.from, nextPlayerMove.to]]);
  }, [showSolution]);

  useEffect(() => {
    if (chapterOrientation.length == 0) return;

    setAvailableBlackStudy(
      !!chapterOrientation.find((x) => x.orientation == "black")
    );
    setAvailableWhiteStudy(
      !!chapterOrientation.find((x) => x.orientation == "white")
    );
  }, [chapterOrientation]);
  ...
  useEffect(() => {
    if (
      ...
      props.hydratedArchives.length == 0 ||
      !chapterFens ||
      chapterFens.length == 0
    )
      return;
  ...
  async function fetchStudy(
    ...
    try {
      let response = await fetch(
        `https://lichess.org/api/study/${studyId}.pgn?comments=false&clocks=false`
      );
    ...
    for (let chapter of chapters) {
      ...
      let chapterSplit = chapter
        ...
        .split("\n\n");
      let cleanedChaperParts = chapterSplit[1].split("("); // Split on "("

      cleanedChaperParts[0] =
        chapterSplit[0].replaceAll("(", "").replaceAll(")", "") +
        "\n\n" +
        cleanedChaperParts[0];
      ...
      while (true) {
        ...
        for (; i < cleanedChaperParts.length; i++) {
          if (!cleanedChaperParts[i].includes(")")) {
            ...
            if (i != cleanedChaperParts.length - 1) {
              ...
              chapterLines[chapterLines.length - 1] = cleanPgn.substring(
                0,
                cleanPgn.length - 1
              );
            }
            ...
            chapterLines[chapterLines.length - 1] +=
              cleanedChaperPartParts.shift(); // Add the end of the line to chapterLines
        ...
        if (cleanedChaperParts.length == 1) {
          // We reduced the array to a single element
          if (chapterLines.length > 1) chapterLines.push(cleanedChaperParts[0]);
          break;
        }
      }

      allChapterLines.push({
    ...
    if (
      localStorage.getItem("studyUrl") == studyUrl &&
      chapterOrientation &&
      chapterOrientation.length > 0
    ) {
    } else if (!orientation) {
      ...
      const chapterOrientation = allChapterLines.map(() => ({
        orientation: orientation,
      }));
      setChapterOrientation(chapterOrientation);
      localStorage.setItem(
        "chapterOrientation",
        JSON.stringify(chapterOrientation)
      );
      localStorage.setItem("studyUrl", studyUrl);
    }
  ...
  function startAsWhite() {
    ...
    let whiteChapters = chapterLines.filter(
      (x, i) => chapterOrientation[i].orientation == "white"
    );
    let rndChapter =
      whiteChapters[Math.floor(Math.random() * whiteChapters.length)];
    if (rndChapter.fen) {
  ...
  function startAsBlack() {
    ...
    let blackChapters = chapterLines.filter(
      (x, i) => chapterOrientation[i].orientation == "black"
    );
    let rndChapter =
      blackChapters[Math.floor(Math.random() * blackChapters.length)];
    if (rndChapter.fen) {
      board.load(rndChapter.fen);
  ...
  function onDrop(sourceSquare: Square, targetSquare: Square) {
    ...
    if (result == null) {
      board.undo();
      setCustomSquareStyles({
        [move.from]: { backgroundColor: "red" },
        [move.to]: { backgroundColor: "red" },
      });
      return true;
    }

    if (move.captured) setCaptureSoundPlay(true);
    else setMoveSoundPlay(true);
  ...
  function findFen(currFen: string, boardOrientation: "white" | "black") {
    ...
    let possibleLinesFiltered = possibleLines.filter(
      ({ iChapter, iLine, iFen }) => chapterFens[iChapter].fens[iLine][iFen + 1]
    );
  ...
  function toggleChapterOrientationItem(i: number) {
    ...
    if (chapterOrientationCopy[i].orientation == "white") newValue = "black";
    else if (chapterOrientationCopy[i].orientation == "black") newValue = null;
    else newValue = "white";
    ...
    localStorage.setItem(
      "chapterOrientation",
      JSON.stringify(chapterOrientationCopy)
    );
  }

  async function setStudyPreset(
    preset:
      | "scotish"
      | "caro-kann"
      | "london-system"
    ...
    switch (preset) {
      case "scotish":
        url = "https://lichess.org/study/iGvRBCsI";
        orientation = "white";
        break;
      case "caro-kann":
        url = "https://lichess.org/study/IZ2Z759V";
        orientation = "black";
        break;
      case "london-system":
        url = "https://lichess.org/study/4sG72RkJ";
        orientation = "white";
        break;
      case "stafford-gambit":
        url = "https://lichess.org/study/whCVdUeM";
        orientation = "black";
        break; // Not on my account
      case "nimzo-bogo-indian":
        url = "https://lichess.org/study/82Wzzapz";
        orientation = "black";
        break; // Not on my account
      case "catalan":
        url = "https://lichess.org/study/x75NW8ek";
        orientation = "white";
        break; // Not on my account
    }

    let allChapterLines = await fetchStudy(url, orientation);
    if (!allChapterLines) return;
  ...
  function compareToGames() {
    if (!props.hydratedArchives) return;
    ...
    for (const archive of props.hydratedArchives) {
      ...
      for (i = 0; i < archive.moves.length; i++) {
        ...
        if (chapterFoundIndex === -1) {
          ...
          prepaSuccess =
            (archive.playingWhite && i % 2 === 1) ||
            (!archive.playingWhite && i % 2 === 0);
      ...
      prepaGameResult.push({
        ...
        chapterName:
          lastChapterFoundIndex != -1
            ? chapterLines[lastChapterFoundIndex].title
            : "",
    ...
    <>
      <Grid direction="column" container className="preparation-container">
        {!studyTitle && !studyLoading ? (
          <>
            ...
            >
              ...
              <Button
                variant="contained"
                aria-label="Fetch Study"
                onClick={() => fetchStudy(studyUrl)}
                disabled={!studyUrl}
              >
                Fetch Study
              </Button>
            ...
            {studyError ? (
              <Alert sx={{ mb: 1 }} severity="error">
                Error fetching study. Make sure it's not private.
              </Alert>
            ) : null}
            <Grid
              container
            ...
            >
              <Button
                ...
                variant="outlined"
                aria-label="Scotish"
                onClick={() => setStudyPreset("scotish")}
              >
                Scotish
              </Button>
              <Button
                ...
                sx={{ mt: 1, ml: 1 }}
                variant="outlined"
                aria-label="Caro-Kann"
                onClick={() => setStudyPreset("caro-kann")}
              >
                Caro-Kann
              </Button>
              <Button
                ...
                sx={{ mt: 1, ml: 1 }}
                variant="outlined"
                aria-label="London System"
                onClick={() => setStudyPreset("london-system")}
              >
                London System
              </Button>
              <Button
                ...
                sx={{ mt: 1, ml: 1 }}
                variant="outlined"
                aria-label="Stafford Gambit"
                onClick={() => setStudyPreset("stafford-gambit")}
              >
                Stafford Gambit
              </Button>
              <Button
                ...
                sx={{ mt: 1, ml: 1 }}
                variant="outlined"
                aria-label="Nimzo/Bogo Indian"
                onClick={() => setStudyPreset("nimzo-bogo-indian")}
              >
                Nimzo/Bogo Indian
              </Button>
              <Button
                ...
                variant="outlined"
                aria-label="Catalan"
                onClick={() => setStudyPreset("catalan")}
              >
                Catalan
              </Button>
            </Grid>
                ...
                <List dense={true}>
                  {chapterLines.map((chapterLine, i) => (
                    <ListItem
                      ...
                      secondaryAction={
                        <IconButton
                          className="chapter-orientation-toggle"
                          color="primary"
                          aria-label="chapter orientation"
                          disabled={!showSuccess && showSuccess !== null}
                          onClick={() => toggleChapterOrientationItem(i)}
                        >
                          {chapterOrientation[i]?.orientation == "white" && (
                            <Tooltip title="Train chapter as white">
                              <img
                                className="side-img"
                                src="./white-king.svg"
                              ></img>
                          ...
                          {chapterOrientation[i]?.orientation == "black" && (
                            <Tooltip title="Train chapter as black">
                              <img
                                className="side-img"
                                src="./black-king.svg"
                              ></img>
                          ...
                          {chapterOrientation[i]?.orientation == null && (
                            <Tooltip title="Chapter disabled">
                              <img
                                className="side-img"
                                style={{
        ...
        <Grid className="chessboard-container">
          ...
          <Grid
            ...
            className={
              ...
              (showSuccess || showSuccess == null ? "visible" : "")
            }
            style={{
              width: Math.min(600, dimensions.width - 30),
              height: Math.min(600, dimensions.width - 30),
            }}
          >
            ...
            >
              <IconButton
                color="primary"
                aria-label="Start as black"
                onClick={startAsBlack}
                disabled={!chapterLines?.length || !availableBlackStudy}
              >
                <img className="start-side-img" src="./black-king.svg"></img>
              </IconButton>
              <IconButton
                color="primary"
                aria-label="Start as white"
                onClick={startAsWhite}
                disabled={!chapterLines?.length || !availableWhiteStudy}
              >
        ...
        >
          ...
          {showTips ? (
            <Tooltip enterDelay={500} title="Show solution">
              <IconButton
                color="primary"
                aria-label="tips"
                component="span"
      ...
      >
        <PreparationTable
          prepaGameResult={prepaGameResult}
          chapterMoves={chapterMoves}
        ></PreparationTable>
      </div>
    </>
  );
